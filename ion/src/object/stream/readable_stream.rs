...
impl ReadableStream {
	...
	pub fn to_object<'cx>(&self, cx: &'cx Context) -> Object<'cx> {
		Object::from(cx.root(self.stream.root(cx).handle().get()))
	}
	...
	pub async unsafe fn read_chunk<'cx>(&self, mut cx: Context) -> ResultExc<Option<Cow<[u8]>>> {
		...
		let chunk = match chunk_val {
			Ok(v) => {
				...
				Object::from(cx.root(v.get().to_object()))
			}
			Err(v) => {
		...
		if obj.get().is_string() {
			let str = crate::String::from(cx.root(obj.get().to_string()));
		...
		let obj_str = crate::String::from(cx.root(ToStringSlow(cx.as_ptr(), obj.as_value(&cx).handle().into())));
		Err(Exception::Error(Error::new(
			format!("Cannot process chunk with unknown type: {}", obj_str.to_owned(&cx)?),
			ErrorKind::Type,
		)))
	}
