...
impl FormDataEntryValue {
	pub fn from_value<'cx: 'v, 'v>(
		...
		if value.get().is_string() {
			...
			&& (Blob::instance_of(cx, &value.to_object(cx)) || File::instance_of(cx, &value.to_object(cx)))
		{
			let obj = value.to_object(cx);
			let file = if File::instance_of(cx, &obj) {
				if let Some(name) = file_name {
					let file = File::get_private(cx, &obj).unwrap();
					cx.root(File::new_object(
						cx,
						Box::new(File::constructor(
							vec![BlobPart(file.blob.as_bytes().clone())],
							name,
							Opt(Some(FileOptions {
								last_modified: Some(Wrap(file.get_last_modified())),
								blob: BlobOptions {
									kind: file.blob.kind(),
				...
				let blob = Blob::get_private(cx, &obj).unwrap();
				cx.root(File::new_object(
					cx,
					Box::new(File::constructor(
			...
			let str = ion::String::from(cx.root(str));
...
impl FormData {
	...
	pub fn append<'cx>(
		&mut self, cx: &'cx Context, name: String, value: ion::Value<'cx>, Opt(file_name): Opt<String>,
	) -> Result<()> {
...
impl JSIterator for FormDataIterator {
	fn next_value<'cx>(&mut self, cx: &'cx Context, private: &ion::Value<'cx>) -> Option<ion::Value<'cx>> {
		...
		let form_data = FormData::get_private(cx, &object).unwrap();
		if self.index >= form_data.kv_pairs.len() {
			...
			match self.mode {
				FormDataIteratorMode::Both => {
					let array = ion::Array::new_with_length(cx, 2);
